...
execution. This model was selected because its six dimensions map one-to-one
...
simultaneously preventing disengagement from internal ideation. \textit{Design
direction:} the cognitive dynamism of divergent thinking, creativity, and
curiosity can be used as a management tool to draw attention from intrinsic
tasks toward the required extrinsic task.
...
anticipation. \textit{Design direction:} the non-conformity, risk-taking,
integrity, and persistence of courage can serve as tools to avoid
procrastination; short-term reward chains replace anxiety-provoking long-term
goals.
...
\textit{Design direction:} replacing priority ordering with a high-meaning goal
narrative activates willpower to begin and sustain priority tasks.
...
others are puzzled or upset. \textit{Design direction:} positive social energy
can be used as an extension tool to maintain positive emotions; behaviour
change requires additional complementary tools.
...
sustained self-regulation~\cite{carter2007}. \textit{Design direction:}
resilience strategies cannot function as a stand-alone module but must be
embedded alongside other adjustment strategies as optional coping choices.
...
\textit{Design direction:} presenting memory-intensive task content in an
artistic form deepens working memory retention.
