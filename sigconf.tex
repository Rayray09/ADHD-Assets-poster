...
recognition for these traits. Brown's executive functioning model~\cite{nigg2005} provides the
complementary deficit side, identifying six dimensions of impairment:
activation, concentration, effort, emotion, memory, and execution.
This model was selected for two reasons. First, it is the most comprehensive
and widely validated multi-dimensional account of how ADHD affects executive
function across adult daily life, covering behavioural, emotional, and
cognitive domains together rather than treating them in isolation. Second,
and critically for this work, its six impairment dimensions map one-to-one
onto the six positive themes identified by Sedgwick et al.\ and Schippers
et al.---a structural correspondence that is not coincidental. Both sides
share the same underlying neural substrates: the same brain regions and
networks (e.g.\ the default mode network, the amygdala, the dorsal attention
network) that produce each executive impairment are also responsible for
generating the corresponding cognitive strength. This means each strength is
not merely \emph{adjacent} to its paired deficit but is produced by the
same neurological mechanism---making it a principled, rather than arbitrary,
compensatory tool. The pairing framework therefore provides a direct
theoretical basis for designing interactions that activate a strength through
its shared neural pathway to redirect the impairment.
